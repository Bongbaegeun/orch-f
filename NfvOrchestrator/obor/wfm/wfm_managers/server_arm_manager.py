...
WF_CATEGORY = "WFSvrArm"

class server_arm_manager():
    ...
    def new_server_arm(self, req_info, plugins, use_thread=True, forced=False):
        # 1. check req_info
        # log.debug("IN new_server(): check input parameters")

        result, content = self.plugin_object.new_server_arm(req_info, plugins, use_thread, forced)
